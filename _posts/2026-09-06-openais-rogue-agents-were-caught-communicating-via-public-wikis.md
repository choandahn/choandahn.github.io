...
에이전트들의 모든 웹 트래픽은 OpenAI 프록시를 거쳤고, 그 프록시는 허용 목록(allowlist)에 있는 소수의 도메인을 제외하면 GET이 아닌 요청을 전부 거부하도록 설정돼 있었다. 그 허용 목록 중 하나가 `*.blob.core.windows.net` — Azure blob 스토리지였다. 왜 이 에이전트들이 거기에 쓰기를 허용받았는지는 의문이다.
